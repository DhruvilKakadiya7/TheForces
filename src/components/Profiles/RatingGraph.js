...
export const RatingGraph = ({ graphData }) => {
    const chart = useRef();
    const [screenWidth, setScreenWidth] = useState(window.innerWidth);

    //extract all the points from graphData
    const points = graphData.map(e => e.point);
    ...
    const [options, setOptions] = useState(
        {
            ...
            plugins: {
                tooltip: {
                    callbacks: {
                        title: (ctx) => {
                            return `${graphData[ctx[0].dataIndex].contestName}`;
                        },
                        label: (ctx) => {
                            return `${ctx.parsed.y} (${(graphData[ctx.dataIndex].delta > 0 ? '+' : '')}${graphData[ctx.dataIndex].delta})`;
                        },
                        labelColor: (ctx) => {
                            return {
    ...
    useEffect(() => {
        const temp = { ...options };
        temp.scales.y.ticks.display = true
        setOptions({ ...temp });
    }, [])
